...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    if _profile:
        ...
        # (prevents HTTP 400/422 on fallback from thinking to non-thinking models).
        api_messages = agent._maybe_strip_reasoning_content(api_messages)
    ...
    # Strip reasoning_content for providers that don't support it
    # (prevents HTTP 400/422 on fallback from thinking to non-thinking models).
    _msgs_for_chat = agent._maybe_strip_reasoning_content(_msgs_for_chat)
...
def _maybe_strip_reasoning_content(
    agent,
    api_messages: list,
) -> list:
    """Strip ``reasoning_content`` from assistant messages when the current
    provider does not support reasoning fields.

    When a thinking model (e.g. GLM-5.1) generates ``reasoning_content`` and
    the agent falls back to a non-thinking provider (e.g. Groq / Llama 3.1),
    the stored assistant messages still carry ``reasoning_content``.  Some
    providers reject this with HTTP 400/422 (``property 'reasoning_content' is
    unsupported``), exhausting the fallback chain.

    Only send ``reasoning_content`` when the current provider is known to
    understand it — DeepSeek thinking mode, Kimi/Moonshot, MiMo, and generally
    reasoning-capable platforms (OpenAI o-series, Anthropic Claude, etc.).
    """
    # ── Gate: does the current provider understand reasoning_content? ──
    needs_reasoning = (
        agent._needs_thinking_reasoning_pad() or agent._supports_reasoning_extra_body()
    )
    if needs_reasoning:
        return api_messages

    # Fast-path: no reasoning_content to strip
    if not any(
        isinstance(msg, dict) and msg.get("reasoning_content") for msg in api_messages
    ):
        return api_messages
