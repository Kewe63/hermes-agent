...
class TestChildCredentialPoolResolution(unittest.TestCase):
    ...
    def test_named_custom_child_uses_requested_provider_pool_when_endpoints_are_shared(self):
        """A named custom provider must not collapse to the first pool that
        happens to share its endpoint with another configured provider."""
        parent = _make_mock_parent()
    ...
    def test_named_custom_child_resolves_shared_endpoint_from_real_config(self):
        """Exercise the real custom-provider lookup against an isolated config."""
        import tempfile
    ...
    def test_build_child_inherits_requested_provider_identity(self, _mock_cfg):
        ...
        self.assertEqual(
            MockAgent.call_args.kwargs["requested_provider"],
            "open-ai",
        )
